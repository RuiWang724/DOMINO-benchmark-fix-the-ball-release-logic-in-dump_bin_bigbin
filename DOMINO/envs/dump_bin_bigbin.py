...
class dump_bin_bigbin(Base_Task):
    ...
    def release_garbage_physics(self) -> bool:
        """Release garbage spheres from kinematic parenting (idempotent)."""
    ...
    def stop_dynamic_object_motion(self) -> bool:
        """Stop deskbin slide on gripper contact, then release garbage physics."""
        ok = super().stop_dynamic_object_motion()
        if ok:
